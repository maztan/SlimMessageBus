    ...
    public class ConsumerInstanceMessageProcessorTest
    {
        ...
        public async Task When_RequestFails_Then_OnMessageFaultIsCalledAndErrorResponseIsSent()
        {
            ...
            _busMock.BusMock.Verify(
                x => x.ProduceResponse(request, It.IsAny<IDictionary<string, string>>(), It.IsAny<SomeResponse>(), It.Is<IDictionary<string, string>>(m => m[ReqRespMessageHeaders.RequestId] == requestId), It.IsAny<ConsumerSettings>()));
