namespace SlimMessageBus.Host.Test
{
    ...
    public class MessageBusTested : MessageBusBase
    {
        ...
        public override Task ProduceToTransport(Type messageType, object message, string path, byte[] messagePayload, IEnumerable<KeyValuePair<string, string>> messageHeaders = null)
        {
            ...
            {
                var req = Settings.Serializer.Deserialize(messageType, messagePayload);
                ...
                var replyTo = messageHeaders?.FirstOrDefault(x => x.Key == ReqRespMessageHeaders.RequestId).Value;
                var resposeHeaders = CreateHeaders();
                resposeHeaders.SetHeader(ReqRespMessageHeaders.RequestId, replyTo);

                var responsePayload = Settings.Serializer.Serialize(resp.GetType(), resp);
